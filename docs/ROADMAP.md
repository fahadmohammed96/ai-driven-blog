...
- [x] **X1 — Researcher** (`ResearchBrief` **effimero** iniettato in `buildPrompt` del Writer + `Proposal.researchContext`; tool interni riusati `retrieveContext`/`getItinerary`/`getMediaForStop`; tool esterno `searchSources` `side:'external'` STUB dietro flag per-tenant `externalResearch` con troncamento `maxOutputTokens`; nessuna tabella/endpoint nuovo). ✓ **suite veloci ALL_GREEN** · SERP-port reale = DEBT-034 (allora nuovo ADR).
- [x] **O1 — Analyst** (lettura `metric_snapshots` cross-canale → `Proposal<PerformanceReport>` tipo `analyst_insight`; aggregazione **deterministica** via tool zero-LLM + sintesi `insights`/`recommendations` tier `balanced`; gate `approve` **acknowledge-only** (nessun `content_item`, default `content_draft` non raggiunto); `mode:'sync'|'batch'` same-schema; idempotenza `tenant\|period\|mode`). ✓ **suite veloci ALL_GREEN** · benchmark statico = DEBT-036 · Batch API reale = DEBT-037 (O0).
- [x] **O2 — Inbound** (triage richieste in ingresso, "ogni richiesta all'umano": classificazione **deterministica** info/lead/reclamo via euristica seed (DEBT-039) → `Proposal<InboundProposal>` tipo `lead_classification`; reply rifinita dall'LLM, tier `fast` salvo `reclamo`→`balanced`; gate `approve` **acknowledge-only / NO-SEND** (nessun `NotificationPort.notify`, nessun `lead` mutato, default `content_draft` non raggiunto); idempotenza `tenant\|message\|leadId`; pipeline CRM Fase 3 invariata). ✓ **suite veloci ALL_GREEN** · classificatore Haiku + `getBookings` reale = DEBT-039.
